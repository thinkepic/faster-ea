	...
	{   
		...
		if($status == 3) {
            ...
            if($level == 'head_of_units') {
                ...
                $target_level = 'finance';
                $email_data = [
                    'approver_name' => $request_detail['fco_monitor_name'],
                    'target_id' => $finance_teams['id'],
                    'target_name' => $finance_teams['username'],
                    'target_email' => $finance_teams['email'],
                ];
            }
            $email_sent = $this->send_approved_email($req_id, $target_level, $email_data);
            if($email_sent) {
                $updated = $this->request->update_status($req_id, $approver_id, $status, $level);
                if($updated) {
